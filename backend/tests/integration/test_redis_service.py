...
class TestRedisService:
    ...
    def test_redis_health_check_passes(self):
        ...
        while time.time() - start_time < timeout:
            ...
            if 'healthy' in result.stdout:
                ...
                assert elapsed < 5, f"Redis took {elapsed:.2f}s to become healthy (expected < 5s)"
                return
            time.sleep(0.5)
    ...
    def test_redis_ping_response(self):
        ...
        assert 'PONG' in result.stdout, f"Redis did not respond to PING. Output: {result.stdout}"
    ...
    def test_redis_volume_exists(self):
        ...
        assert len(redis_volumes) > 0, f"Redis volume does not exist. Found volumes: {volumes}"
    ...
    def test_redis_maxmemory_configured(self):
        ...
        if len(lines) >= 2:
            ...
            assert maxmemory == expected, f"Redis maxmemory is {maxmemory}, expected {expected}"
    ...
    def test_redis_eviction_policy_configured(self):
        ...
        if len(lines) >= 2:
            policy = lines[1]
            assert policy == 'allkeys-lru', f"Redis maxmemory-policy is {policy}, expected allkeys-lru"
